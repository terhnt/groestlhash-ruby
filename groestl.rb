    ...
    def int32Buffer2Bytes(b)
	  buffer = Array.new(b.length)
	  len = b.length
	  i = 0
	  while (i < len) do
		buffer[i * 4] = unsignedRightShift((b[i] & 0xFF000000),24)
    ...
    def int32ArrayToHexString(array)
	  string = ''
	  len = array.length
	  for i in 0..len do
	    s = array[i]
    ...
    def bytes2Int64Buffer(b)
      ...
      len = b.length ? ((unsignedRightShift((b.length - 1), 3)) + 1) : 0
      buffer = Array.new(len)
      j = 0
      while j < len do
    ...
    def b64Decode(input)
      ...
      input = input.gsub! /[^A-Za-z0-9\+\/\=]/g, ""
      
      while i < input.length do
        enc1 = keyStr.index(input[i++].chr)
        enc2 = keyStr.index(input[i++].chr)
        enc3 = keyStr.index(input[i++].chr)
        enc4 = keyStr.index(input[i++].chr)
    ...
    def bufferInsert(buffer, bufferOffset, data, len, dataOffset)
      dataOFfste = dataOffset | 0
      i = 0
      while i < len do
    ...
    def setShiftLeft(bits)
      ...
      if (bits >31)
        ...
        toMoveUp = unsignedRightShift(this.lo, 32 - bits)
        this.lo <<= bits
    ...
    def groestl(str, format, output)
	  a = xgroestl(str, format, 2)
	  a = a.slice(0,8)
	  if (output === 2)
	    return a
	  end
